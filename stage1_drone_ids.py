...
def generate_large_flight_dataset(
    ...
    """Generate a realistic normal-flight telemetry dataset."""

    output_path.parent.mkdir(parents=True, exist_ok=True)

    duration_s = max(120.0, target_distance_km * 450.0)
    sample_period_s = 0.1
    ...
    count = int(duration_s / sample_period_s)

    for i in range(count):

        t = i * sample_period_s

        # Realistic multirotor ground speed
        ground_speed = (
            8.0
            + 0.45 * math.sin(t / 3.5)
        ...
        # GPS speed normally stays close to ground speed
        gps_speed = ground_speed + rng.uniform(-0.12, 0.12)

        # Smooth heading variation
        heading = (
            35.0
            + 4.0 * math.sin(t / 5.0)
            + rng.uniform(-0.3, 0.3)
        ...
        # Convert movement into north/east displacement
        north_m = (
            ground_speed
            * sample_period_s
        ...
        record = DroneTelemetry(
            ...
            satellites=12 + rng.choice([-1, 0, 0, 0, 1]),
            hdop=max(0.55, 0.90 + rng.uniform(-0.08, 0.08)),
            roll=1.5 * math.sin(t / 2.5),
            pitch=1.2 * math.sin(t / 3.0),
            battery_voltage=16.4 - 0.005 * t,
            flight_mode="GUIDED",
            message_type="GLOBAL_POSITION_INT",
            message_rate=10.0 + rng.uniform(-0.3, 0.3),
            command_count=i // 10,
            source="generated_flight_dataset",
            scenario="NORMAL",
    ...
    print(
        f"[PASS] Generated {len(records)} telemetry frames "
        f"for approximately {target_distance_km:.1f} km flight."
    )
def main() -> int:
    parser = build_parser()
